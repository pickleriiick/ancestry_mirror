...
if RMAMBIG==True:
	...
	toprint2 = dict()
	for p, f in toprint.items():
        	if re.search("AMBIG", p) == None:
			toprint2[p] = f
			t = t+f
	for p, f in toprint2.items():
        	toprint2[p] = f/t
	print >> outfile, json.dumps(toprint2, indent = 4)
else:
	print >> outfile, json.dumps(toprint, indent = 4)
