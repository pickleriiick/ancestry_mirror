...
double FreqData::get_weighted_f_post(pair<string, int> cpos, map<string, float> ancestry, bool obs){
	double toreturn = 0;
	for (map<string, float>::iterator it = ancestry.begin(); it != ancestry.end(); it++){
		double f = get_f_post(cpos, it->first, obs, it->second);
		double wf = f*it->second;
		toreturn +=wf;
	}
	return toreturn;
...
double FreqData::get_f_post(pair<string, int> cpos, string pop, bool d, float weight){
	string chr = cpos.first;
	int pos = cpos.second;
	if (chr2pos2index.find(chr) != chr2pos2index.end() and chr2pos2index[chr].find(pos) != chr2pos2index[chr].end()){
		int snpindex = chr2pos2index[chr][pos];
		int popindex = pop2id[pop];
		double count1 = (double) allele_counts[snpindex][popindex].first;
		double count2 = (double) allele_counts[snpindex][popindex].second;
		if (d) count1 += weight;
		count2+=weight;
...
double FreqData::lbinom(int k, double p, int n){
	double toreturn = log(binomial_coefficient<long double>(n, k));
