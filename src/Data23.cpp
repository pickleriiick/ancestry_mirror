...
void Data23::printVCF(string outfile, map<string, float> weights){
	...
	set<string> seenchrs;
	kstring_t str = {0,0,0};
	bcf_hdr_append(hdr, "##source=ancestry");
	for (vector<pair<string, int> >::iterator it = info.begin(); it != info.end(); it++) {
		string tmp = it->first;
		if (seenchrs.find(tmp) == seenchrs.end() ){
			string tmpstr = "##contig=<ID="+tmp+">";
			bcf_hdr_append(hdr, tmpstr.c_str());
			seenchrs.insert(tmp);
		}
	}
	bcf_hdr_append(hdr, "##phasing=unphased");
	bcf_hdr_append(hdr, "##note=reference and alt alleles are defined relative to major and minor alleles in the ancestry reference panel");
	bcf_hdr_append(hdr, "##FORMAT=<ID=GT,Number=1,Type=String,Description=\"Genotype\">");
	bcf_hdr_append(hdr, "##FORMAT=<ID=RC,Number=1,Type=Integer,Description=\"Count of reads with MAJOR/REF allele\">");
	bcf_hdr_append(hdr, "##FORMAT=<ID=AC,Number=1,Type=Integer,Description=\"Count of reads with MINOR/ALT allele\">");
	bcf_hdr_append(hdr, "##FORMAT=<ID=IAF,Number=1,Type=Float,Description=\"Model-based estimate of the individual allele frequency for the minor/alt allele\">");
	bcf_hdr_add_sample(hdr, params->infile.c_str());
	bcf_hdr_add_sample(hdr, NULL);      // to update internal structures
	bcf_hdr_write(fp, hdr);

	for (int i = 0; i < nsnp; i++){
		bool g = data[i];
		double tmpf = ref->get_weighted_f_post(info.at(i), weights, g);
		// Add a record
		// 20     14370   rs6054257 G      A       29   PASS   NS=3;DP=14;AF=0.5;DB;H2           GT:GQ:DP:HQ 0|0:48:1:51,51 1|0:48:8:51,51 1/1:43:5:.,.
		// .. CHROM
		bcf_clear1(rec);
		pair<string, int> tinfo = info[i];

		string chr = tinfo.first;
		int pos = tinfo.second;
		stringstream ss;
		ss << pos;
		string posstr = ss.str();
		rec->rid = bcf_hdr_name2id(hdr, chr.c_str());
		pair<string, string> alleles = ref->get_als(make_pair(chr, pos));
		// these alleles are minor/major, so lets put minor as alt
		string aa = alleles.second+","+alleles.first;
		// .. POS
		rec->pos = pos-1; //0-based
		// .. ID
		string tmpid = chr+":"+posstr+":"+alleles.second+":"+alleles.first;
		...
		float *tmpfa = (float*)malloc(bcf_hdr_nsamples(hdr)*2*sizeof(float));
		//cout <<  "setting depth\n"; cout.flush();
		// sequencing depth
		int rc = 0;
		int ac = 0;
		...
		tmpfa[0] = tmpf;
		//cout <<  "setting iaf1\n"; cout.flush();
		bcf_update_format_float(hdr, rec, "IAF", &tmpfa[0], bcf_hdr_nsamples(hdr));
		//cout <<  "setting iaf2\n"; cout.flush();
...
bool Data23::mergeclusters(){
	cout << "trying merge\n"; cout.flush();
	double initllk = llk();
	double maxmerge = -100000000.0;
	int merge_from, merge_to;
	string strmerge_from, strmerge_to;
	for (int i = 0; i < npop; i++){
		for (int j = 0; j < npop; j++){
			if (i == j) continue;
			double tmplk = try_droppop(i, j);
			if (tmplk > maxmerge){
				maxmerge = tmplk;
				merge_from = i;
				merge_to = j;
				strmerge_from = refpopnames[i];
				strmerge_to = refpopnames[j];
			}
		}
	}

	cout << initllk-maxmerge << " "<< strmerge_from << " "<< strmerge_to << "\n";
	double test =initllk-maxmerge;
	if (test < params->mergelk){
		double oldfrom = gsl_vector_get(Q, merge_from);
		double oldto = gsl_vector_get(Q, merge_to);
		gsl_vector_set(Q, merge_from, 1e-6);
		gsl_vector_set(Q, merge_to, oldfrom+oldto);
		dropuseless();
		return true;
	}
	return false;
}
double Data23::try_droppop(int from, int to){
	//
	// try merging parts of the Q vector
	//
	double oldfrom = gsl_vector_get(Q, from);
	double oldto = gsl_vector_get(Q, to);
	gsl_vector_set(Q, from, 1e-6);
	gsl_vector_set(Q, to, oldfrom+oldto);
	double tmplk = llk();
	gsl_vector_set(Q, from, oldfrom);
	gsl_vector_set(Q, to, oldto);
	return tmplk;
...
map<string, float> Data23::return_Q(){
	map<string, float> toreturn;
	for (int i = 0; i < npop; i++){
		toreturn[refpopnames[i]] = gsl_vector_get(Q, i) ;
	}
	return toreturn;
}
